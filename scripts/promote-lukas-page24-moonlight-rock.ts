...
const PAGE_24_LOCAL = path.join(process.cwd(), 'tmp/page24-fix/p24-moonlight-rock-manual-strong.png');
const PAGE_24_EXPECTED_MD5 = '205f1eb355d53787e394fc710b3efb77';
const PAGE_24_BLOB_NAME = 'canonical-page24-moonlight-rock/page-24-p24-bedroom-moonlight-rock.png';
...
async function main() {
  ...
  const proofLocal = path.join(outDir, 'lukas-page24-moonlight-rock-proof-local.pdf');
  const interiorLocal = path.join(outDir, 'lukas-page24-moonlight-rock-interior-local.pdf');
  await writeFile(proofLocal, proof);
  await writeFile(interiorLocal, interior);

  const proofUrl = await putBlob(
    `orders/${ORDER_ID}/lukas-page24-moonlight-rock-proof-2026-05-06-canonical.pdf`,
    proof,
    'application/pdf',
    apply,
    token,
  ...
  const interiorUrl = await putBlob(
    `orders/${ORDER_ID}/lukas-page24-moonlight-rock-interior-2026-05-06-canonical.pdf`,
    interior,
    'application/pdf',
    apply,
    token,
  ...
  const updated: OrderRecord = {
    ...
    auditEvents: [
      ...(order.auditEvents ?? []),
      {
        at: now,
        type: 'proof_rebuilt',
        reason: 'promote_lukas_page24_moonlight_rock_no_lulu',
        meta: {
          page24Url,
          page24Md5,
          proofUrl,
  ...
  const backupPath = path.join(backupDir, `${ORDER_ID}-pre-page24-moonlight-rock-${safeTs}.json`);
  ...
  if (!apply) {
    console.log('[promote-lukas-page24-moonlight-rock] dry-run only; pass --apply to upload/persist.');
    return;
  }

  await writeFile(backupPath, `${JSON.stringify(order, null, 2)}\n`, 'utf8');
  await persistOrder(updated);
  console.log('[promote-lukas-page24-moonlight-rock] applied');
}

main().catch((err) => {
  console.error('[promote-lukas-page24-moonlight-rock] failed:', err instanceof Error ? err.message : err);
