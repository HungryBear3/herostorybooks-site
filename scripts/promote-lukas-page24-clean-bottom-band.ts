...
const PAGE_24_LOCAL = path.join(process.cwd(), 'tmp/page24-fix/p24-seedream-from-original-clean.png');
const PAGE_24_EXPECTED_MD5 = '3e4658c1012285682af3197dbd135223';
const PAGE_24_BLOB_NAME = 'canonical-page24-clean/page-24-p24-bedroom-window-moon-clean.png';
...
async function main() {
  ...
  const proofLocal = path.join(outDir, 'lukas-page24-clean-bottom-band-proof-local.pdf');
  const interiorLocal = path.join(outDir, 'lukas-page24-clean-bottom-band-interior-local.pdf');
  await writeFile(proofLocal, proof);
  await writeFile(interiorLocal, interior);

  const proofUrl = await putBlob(
    `orders/${ORDER_ID}/lukas-page24-clean-bottom-band-proof-2026-05-06-canonical.pdf`,
    proof,
    'application/pdf',
    apply,
    token,
  ...
  const interiorUrl = await putBlob(
    `orders/${ORDER_ID}/lukas-page24-clean-bottom-band-interior-2026-05-06-canonical.pdf`,
    interior,
    'application/pdf',
    apply,
    token,
  ...
  const updated: OrderRecord = {
    ...
    auditEvents: [
      ...(order.auditEvents ?? []),
      {
        at: now,
        type: 'proof_rebuilt',
        reason: 'promote_lukas_page24_clean_bottom_band_no_lulu',
        meta: {
          page24Url,
          page24Md5,
          proofUrl,
  ...
  const backupPath = path.join(backupDir, `${ORDER_ID}-pre-page24-clean-bottom-band-${safeTs}.json`);
  ...
  if (!apply) {
    console.log('[promote-lukas-page24-clean-bottom-band] dry-run only; pass --apply to upload/persist.');
    return;
  }

  await writeFile(backupPath, `${JSON.stringify(order, null, 2)}\n`, 'utf8');
  await persistOrder(updated);
  console.log('[promote-lukas-page24-clean-bottom-band] applied');
}

main().catch((err) => {
  console.error('[promote-lukas-page24-clean-bottom-band] failed:', err instanceof Error ? err.message : err);
