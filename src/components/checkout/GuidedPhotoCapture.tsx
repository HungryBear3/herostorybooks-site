...
export function GuidedPhotoCapture({
  ...
  const capturedLabels = new Set(frames.map((frame) => sanitizeGuidedPhotoLabel(frame.label)));
  const completed = GUIDED_PHOTO_PROMPTS.filter((prompt) => capturedLabels.has(prompt.label)).length;
  const allRequiredCaptured = completed === GUIDED_PHOTO_PROMPTS.length;
      ...
      <div className="grid gap-4 md:grid-cols-[1fr_0.85fr]">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {allRequiredCaptured ? "All required photos captured" : <>Take photo{currentPrompt?.title ? ` · ${currentPrompt.title}` : ""}</>}
            </button>
          </div>
        ...
        <div className="space-y-3">
          <div className="rounded-xl border border-[#cfe0d8] bg-white/55 p-3">
            <p className="text-sm font-bold text-[#1f1a16]">
              {allRequiredCaptured
                ? "All required still reference photos are ready"
                : `Step ${Math.min(currentPromptIndex + 1, GUIDED_PHOTO_PROMPTS.length)} of ${GUIDED_PHOTO_PROMPTS.length}: ${currentPrompt?.title ?? "Reference photo"}`}
            </p>
            <p className="mt-1 text-xs leading-5 text-[#5f766f]">
              {allRequiredCaptured
                ? "You can remove or retake any still before checkout. If you remove one, this guide jumps back to that missing angle."
