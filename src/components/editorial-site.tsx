...
import { PUBLIC_CATALOG, type PublicProductId } from '@/lib/public-catalog';
import { PUBLIC_HOME_FAQS } from '@/lib/public-faqs';
...
/** Display tier id -> public contract format id. */
const CATALOG_PRODUCT_ID: Record<TierId, PublicProductId> = {
  digital: 'digital',
  softcover: 'classic',
  hardcover: 'premium',
};

function catalogPrice(id: TierId): number {
  const product = PUBLIC_CATALOG.products.find((p) => p.id === CATALOG_PRODUCT_ID[id]);
  if (!product) throw new Error(`No public catalog product for tier ${id}`);
  return product.priceMinorUnits / 100;
}

// Canonical pricing now comes from src/lib/public-catalog.ts, which is also
// what the public JSON API, the JSON-LD, and /llms.txt serve. The tier ids
// here are the display ids this layout has always used; `catalogPrice` maps
// them onto the contract's format ids, so a price can no longer be changed on
// one surface and not the others. tests/public-catalog.test.ts asserts the
// contract still equals src/lib/orders.ts FORMAT_META.priceCents, i.e. what
// Stripe actually charges.
const tiers: Tier[] = [
  {
    id: 'digital',
    name: 'Digital PDF',
    price: catalogPrice('digital'),
  ...
  return (
    <section id="faq" className="mx-auto max-w-4xl px-5 py-20 md:px-8">
      ...
      <div className="space-y-4">
        {PUBLIC_HOME_FAQS.map(([q, a]) => (
          <details key={q} className="group rounded-2xl border border-[#d8c6a2] bg-[#fff8ec] p-6" open={q === PUBLIC_HOME_FAQS[0][0]}>
