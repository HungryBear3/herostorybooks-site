...
import { findFulfillmentBacklog, runFulfillmentSweep } from '../src/lib/fulfillment-backlog.ts';
...
test('sweep refuses before listing orders when unauthorized', async () => {
  let listed = false;
  let triggered = false;
...
test('sweep triggers only bounded paid not_started backlog', async () => {
  const triggered: string[] = [];
  const out = await runFulfillmentSweep({
    authorized: true,
    listOrders: async () => [
      order({ id: 'ord_oldest', fulfillmentStatus: 'not_started', createdAt: '2026-06-04T09:00:00.000Z', updatedAt: '2026-06-04T09:00:00.000Z' }),
      order({ id: 'ord_running', fulfillmentStatus: 'generating_story' }),
      order({ id: 'ord_second', fulfillmentStatus: 'not_started', createdAt: '2026-06-04T10:00:00.000Z', updatedAt: '2026-06-04T10:00:00.000Z' }),
    ],
  ...
  assert.deepEqual(triggered, ['ord_oldest']);
  assert.deepEqual(out.body.results, [{ orderId: 'ord_oldest', status: 'started' }]);
});
