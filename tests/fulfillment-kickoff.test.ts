...
test('scheduler-5: source-level — direct fulfillmentStatus=complete writes are bounded and accounted for', () => {
  ...
  // admin-actions.ts has ONE legitimate completion write: the dedicated
  // resendDigitalDelivery handle recovers an order from
  // fulfillmentStatus='delivery_email_failed' after a successful customer
  // email re-send. Generic retry intentionally fails closed for this state.
  ...
  assert.equal(
    ...
    1,
    `admin-actions.ts is allowed exactly 1 fulfillmentStatus=complete write (dedicated delivery_email_failed recovery), got ${adminCompleteAssignments.length}`,
  );
