...
test('upgrade offer calculates delta and uses print_upgrade Stripe metadata', () => {
  assert.equal(calculatePrintUpgradeDeltaCents('digital', 'classic'), 3000);
  assert.equal(calculatePrintUpgradeDeltaCents('digital', 'premium'), 5000);

  const offer = buildPrintUpgradeOffer(BASE_DIGITAL_ORDER, 'classic');
  assert.equal(offer.deltaCents, 3000);
...
test('admin print-upgrade API defaults to dry-run preview and requires explicit checkout confirmation', () => {
  ...
  assert.match(route, /No Stripe Checkout Session created\. No email sent\. No print\/provider action triggered\./);
