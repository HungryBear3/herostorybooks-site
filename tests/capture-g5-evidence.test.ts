...
const FAKE = {
  STRIPE_SECRET_KEY: 'sk_' + 'live_' + 'TEST_REDACTION_ONLY_1234567890ABCDEF',
  STRIPE_WEBHOOK_SECRET: 'wh' + 'sec_' + 'DEADBEEFdeadbeefSECRETwebhookbody99',
  BLOB_READ_WRITE_TOKEN: 'vercel_blob_' + 'rw_PRIVATEtokenBODYshouldNeverShow',
  HSB_ORDER_ADMIN_KEY: 'super-secret-admin-key-no-prefix-9988776655',
  RESEND_API_KEY: 're_' + 'LIVERESENDkeyBODYsecret0001',
};
...
test('redactSecret: known-format secret reveals only length + whitelisted prefix, never the body', () => {
  ...
  assert.equal(r.prefix, 'sk_live_');
...
test('redactSecret: every emitted prefix is from the fixed whitelist', () => {
  for (const sample of ['sk_test_abc', 'whsec_abc', 're_abc', 'vercel_blob_rw_abc', 'pk_live_abc']) {
...
test('CLI run: writes a packet, exits 0 even with blockers, and emits no injected secret', () => {
  ...
  try {
    ...
    assert.match(json, /"prefix": "sk_live_"/);
